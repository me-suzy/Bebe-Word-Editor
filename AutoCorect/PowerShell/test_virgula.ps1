# Script de test pentru verificarea separării cuvintelor prin virgulă
# Autor: Assistant AI
# Data: 27 august 2025

Write-Host "=== TEST SEPARARE CUVINTE PRIN VIRGULĂ ===" -ForegroundColor Green
Write-Host "Testează separarea cuvintelor lipite cu virgulă..." -ForegroundColor Yellow

# 1. Creează fișierul de test
$outputFile = "test_virgula.txt"
...
function Separate-Words-With-Commas {
    ...
    $result = $result -replace '([a-zA-ZăâîșțĂÂÎȘȚ]{6,})([a-zA-ZăâîșțĂÂÎȘȚ]{6,})', '$1,$2'
...
Write-Host "`nTestul este complet! Verifică fișierul test_virgula.txt pentru a vedea separarea cu virgulă." -ForegroundColor Cyan
