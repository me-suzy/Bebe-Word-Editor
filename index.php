...
<head>
    ...
    <style>
        * {
        ...
        /* PIN — fixează fișierul curent în lista de la pornire */
        .topbtn.pin-btn {
            min-width: 34px;
            padding-left: 8px;
            padding-right: 8px;
        ...
        .topbtn.pin-btn.pinned {
            background: #f2c14e;
            border-color: #d9a326;
            color: #3a2c00;
        }

        .topbtn.pin-btn:hover {
            filter: brightness(1.08);
        }
        ...
        .file-item .unpin:hover {
            color: #a4262c;
        }

        /* liste derulabile în sidebar (PIN + recente) */
        #sbPinned,
        #sbRecent {
            max-height: 170px;
            overflow-y: auto;
        ...
        #sbPinned::-webkit-scrollbar,
        #sbRecent::-webkit-scrollbar {
            width: 9px;
        }

        #sbPinned::-webkit-scrollbar-thumb,
        #sbRecent::-webkit-scrollbar-thumb {
            background: #c8c6c4;
            border-radius: 5px;
        }

        #sbPinned::-webkit-scrollbar-thumb:hover,
        #sbRecent::-webkit-scrollbar-thumb:hover {
            background: #a19f9d;
        }
        ...
        /* ── PIN: fișiere fixate, deasupra celor recente ── */
        .pin-section {
            margin-bottom: 12px;
            padding-bottom: 10px;
        ...
        /* liste derulabile: PIN-uri + minim 20 de fișiere recente la un click distanță */
        #pinnedList {
            max-height: 160px;
            overflow-y: auto;
        }
        ...
        #pinnedList::-webkit-scrollbar-track,
        ...
        #pinnedList::-webkit-scrollbar-thumb:hover,
...
<body>
    ...
    <div class="topbar">
        ...
        <button class="topbtn pin-btn" id="pinBtn" onclick="togglePinCurrent()" title="PIN">📌</button>
    ...
    <div class="main">
        <div class="sidebar" id="sidebar">
            ...
            <h3 id="sbPinnedHdr" style="display:none">📌 Fixate (PIN)</h3>
            <div id="sbPinned"></div>
        ...
        <div class="overlay-card">
            ...
            <div class="overlay-row">
                ...
                <div class="recent-panel" id="recentPanel" style="display:none">
                    <!-- PIN: fișierele fixate rămân aici la fiecare pornire, deasupra celor recente -->
                    <div class="pin-section" id="pinnedSection" style="display:none">
                        <h5>📌 Fișiere fixate (PIN)</h5>
                        <div id="pinnedList"></div>
                    </div>
                    <h5 id="recentHdr">Fișiere recente</h5>
    ...
    <script>
        ...
        // Enter să creeze <p>, nu <div> (altfel paragrafele noi nu au stilurile/alinierea de <p>)
        ...
        // Un „paragraf" valid = element bloc. Textul sau elementele inline (span, b, a…)
        // ajunse DIRECT într-o coală .page (din fișiere salvate anterior, paste peste
        // paragrafe etc.) nu sunt văzute de getBlock/blockOf → alinierea și Format
        // Painter eșuau silențios pe ele. Le înfășurăm în <p>, grupând nodurile
        // inline consecutive (fac parte din același paragraf logic).
        const BLOCK_TAG_RE = /^(P|H[1-6]|LI|UL|OL|BLOCKQUOTE|PRE|DIV|TABLE|HR|FIGURE)$/;
        function isBlockNode(n) { return n.nodeType === 1 && BLOCK_TAG_RE.test(n.tagName); }
        function isStrayInline(n) {
            return (n.nodeType === 3 && n.nodeValue.trim() !== '') || (n.nodeType === 1 && !BLOCK_TAG_RE.test(n.tagName));
        }
        // verificare ieftină: merită să calculăm offset-urile selecției?
        function hasSheetStrays() {
            const sheets = page.querySelectorAll('.page');
            for (const sh of sheets)
                for (let n = sh.firstChild; n; n = n.nextSibling)
                    if (isStrayInline(n)) return true;
            return false;
        }
        function normalizeSheetStrays() {
            ...
            page.querySelectorAll('.page').forEach(sh => {
                ...
                while (n) {
                    ...
                    sh.insertBefore(p, n);
                    let m = p.nextSibling;
                    while (m && !isBlockNode(m)) { const next = m.nextSibling; p.appendChild(m); m = next; }
                    changed = true;
                    n = p.nextSibling;
                }
            });
        ...
        function setSelectionByOffsets(start, end) {
            ...
            let count = 0, node, sN = null, sO = 0, eN = null, eO = 0;
            while ((node = w.nextNode())) {
                const len = node.nodeValue.length;
                if (!sN && count + len >= start) { sN = node; sO = start - count; }
                if (sN && count + len >= end) { eN = node; eO = end - count; break; }
                count += len;
            }
            if (!sN) return false;
            if (!eN) { eN = sN; eO = sN.nodeValue.length; }
        ...
        // normalizează + păstrează selecția (offset-urile de caractere nu se schimbă la înfășurare)
        ...
        function setDocHtml(html) {
            ...
            const blocks = [];
            let pendingP = null;   // adună nodurile text/inline consecutive într-un singur <p>
            src.childNodes.forEach(n => {
                if (n.nodeType === 3 && !n.nodeValue.trim()) return;
                if (n.nodeType === 3 || (n.nodeType === 1 && !BLOCK_TAG_RE.test(n.tagName))) {
                    if (!pendingP) { pendingP = document.createElement('p'); blocks.push(pendingP); }
                    pendingP.appendChild(n.cloneNode(true));
                    return;
                }
                pendingP = null;
                blocks.push(n);
        ...
        function repaginate() {
            // Use the caret-preserving path, and read the current document when the
            // timer runs. Rebuilding a captured HTML snapshot resets the caret to 0
            // and can also discard text pasted before the timer fires.
            saveEditorSelection();
            scheduleEditorRepaginate(10);
        }

        let autoRepaginateTimer = null;
        let autoRepaginating = false;
        function insertPaginationCaretMarker() {
            // A fresh click or paste can move the live caret before selectionchange.
            // Do not replace that destination with an older saved range.
            if (!saveEditorSelection()) restoreEditorSelection();
            ...
            r.collapse(true);
            // caret „în" un element void (<br>, <img>) → marcajul ar dispărea la serializare
            const sc = r.startContainer;
            if (sc.nodeType === 1 && /^(BR|IMG|HR|INPUT|WBR)$/.test(sc.tagName) && sc.parentNode) r.setStartBefore(sc);
            r.insertNode(marker);
            return true;
        }
        ...
        function renderSidebarRecent() {
            ...
            items.forEach(e => {   // lista are scroll propriu → le arătăm pe toate
                ...
                el.innerHTML = '<span class="ico">' + fileIcon(ext) + '</span><span class="fn">' + escapeHtml(e.name) + '</span>';
        ...
        // Chrome mută caretul la ÎNCEPUTUL zonei editabile când #pages primește din nou
        // focusul fără un clic în text (ieșire dintr-un câmp al ribbonului, revenire în
        // fereastră etc.). Selecția „bună" era apoi suprascrisă de selectionchange, așa că
        // un Ctrl+V lipea textul înaintea primei linii. Reținem caretul la blur și îl punem
        // înapoi la focus — dar numai dacă browserul chiar l-a dus la începutul documentului.
        let caretBeforeBlur = null;
        let pointerInsidePage = false;
        function selectionAtEditorStart(range) {
            if (!range || !range.collapsed) return false;
            const pre = document.createRange();
            pre.selectNodeContents(page);
            try { pre.setEnd(range.startContainer, range.startOffset); } catch (e) { return false; }
            return pre.toString().length === 0;
        }
        page.addEventListener('pointerdown', () => { pointerInsidePage = true; }, true);
        page.addEventListener('mousedown', () => { pointerInsidePage = true; }, true);
        document.addEventListener('pointerup', () => { pointerInsidePage = false; }, true);
        document.addEventListener('mouseup', () => { pointerInsidePage = false; }, true);
        page.addEventListener('blur', () => {
            const live = selectionRangeInsidePage() || editorSavedRange;
            caretBeforeBlur = live ? live.cloneRange() : null;
        });
        page.addEventListener('focus', () => {
            const saved = caretBeforeBlur;
            caretBeforeBlur = null;
            if (!saved || pointerInsidePage) return;      // clicul în text își pune singur caretul
            if (!page.contains(saved.startContainer) || !page.contains(saved.endContainer)) return;
            const live = selectionRangeInsidePage();
            if (live && !selectionAtEditorStart(live)) return;   // browserul a păstrat o poziție reală
            try {
                const sel = window.getSelection();
                sel.removeAllRanges();
                sel.addRange(saved.cloneRange());
                editorSavedRange = saved.cloneRange();
            } catch (e) { }
        });
        ...
        page.addEventListener('beforeinput', noteTypingSnapshot);
        ...
        function getBlock() {
            ...
            while (n && n !== page && !(n.nodeType === 1 && !n.classList.contains('page')
                && /^(P|H1|H2|H3|H4|H5|H6|LI|BLOCKQUOTE|PRE|DIV)$/.test(n.tagName)))
                n = n.parentNode;
            return (n && n !== page) ? n : null;
        }
        ...
        // ── Format Painter ── copiază stilul ÎNTREGULUI paragraf-sursă (nu doar al
        // cuvântului de sub cursor), apoi:
        //   • CLICK simplu pe țintă (fără tragere)  → înlocuiește formatarea pe TOT paragraful
        ...
        // stilul „dominant" al paragrafului = stilul care acoperă cele mai multe caractere
        // (ex: paragraf normal cu un cuvânt bold → stilul normal, nu bold-ul)
        function dominantStyleElement(block) {
            ...
            while ((tn = w.nextNode())) {
                ...
                const cs = getComputedStyle(el);
                const key = [cs.fontFamily, cs.fontSize, cs.color, cs.fontWeight, cs.fontStyle, cs.textDecorationLine, cs.backgroundColor].join('|');
                const t = totals.get(key) || { el, len: 0 };
                t.len += len; totals.set(key, t);
            }
            let best = null;
        ...
        function toggleFormatPainter() {
            ...
            // preia stilul întregului paragraf-sursă (nu doar al cuvântului de sub cursor)
        ...
        // Format Painter ÎNLOCUIEȘTE formatarea de caracter, nu se adaugă peste ea (ca în Word).
        // Fără curățare, fiecare aplicare mai adăuga un strat de <span>, iar <u>/<s> rămâneau
        // active: text-decoration moștenit nu poate fi anulat de un descendent.
        ...
        function stripCharFormatting(block) {
            ...
            [...block.querySelectorAll('*')].forEach(el => {
                ...
                if (el.tagName !== 'SPAN') { unwrapElement(el); return; }
        ...
        function applyPainter(selectionOnly) {
            ...
            ensureBlockStructure();   // țintele fără <p> învelitor (text/span direct în coală) devin paragrafe reale
            let range = sel.getRangeAt(0);
            const useSel = selectionOnly && !range.collapsed && sel.toString().trim().length > 0;
            let targetBlocks = [];
            if (!useSel) {
                const blk = getBlock(); if (!blk) { disarmPainter(); toast('Click în interiorul unui paragraf'); return false; }
                targetBlocks = [blk];
            ...
            if (!useSel) {
                // click pe paragraf → înlocuiește formatarea veche, apoi aplică stilul-sursă
                targetBlocks.forEach(b => { stripCharFormatting(b); FONT_PROPS.forEach(p => b.style.removeProperty(p)); });
                range = document.createRange(); range.selectNodeContents(targetBlocks[0]);
            }
        ...
        // ── Paste: text curat, cu formatarea paragrafului-destinație ──
        // Ca în Word la lipirea de text simplu: paragraful în care lipim își păstrează
        // alinierea și fontul, chiar dacă îi înlocuim tot conținutul. Fără asta,
        // paragraful lipit rămânea nestilat (fără justify) peste unul formatat.
        function destinationFormat() {
            ...
            if (styled && styled !== blk) {
                ...
                css = 'font-family:' + cs.fontFamily + ';font-size:' + cs.fontSize + ';color:' + cs.color
        ...
        page.addEventListener('paste', (e) => {
            ...
            if (!text) return;
            if (!saveEditorSelection() && !restoreEditorSelection()) return;
            const fmt = destinationFormat();
            // font-family poate conține ghilimele („Times New Roman") → obligatoriu escapate,
            // altfel atributul style="…" se închide devreme și toată formatarea se pierde
            const open = '<p' + (fmt.align && fmt.align !== 'left' ? ' style="text-align:' + fmt.align + '"' : '') + '>';
            const cssAttr = fmt.css.replace(/&/g, '&amp;').replace(/"/g, '&quot;');
            const html = text.split(/\r?\n\r?\n/).map(par => {
                let body = escapeHtml(par).replace(/\r?\n/g, '<br>');
                if (fmt.css) body = '<span style="' + cssAttr + '">' + body + '</span>';
                return open + body + '</p>';
            }).join('');
            // insertHTML NU declanșează 'beforeinput' în Chrome, deci snapshot-ul de undo
            // trebuie luat aici explicit — altfel Ctrl+Z după lipire zicea „Nimic de anulat".
            recordUndo(); suppressSnap = true;
            try {
                document.execCommand('insertHTML', false, html || '<p><br></p>');
                ensureBlockStructure();  // execCommand poate lăsa text/span direct în coală
            } finally { suppressSnap = false; }
            // Keep the post-paste caret before a queued pagination can run.
            // (restul — word count, autosave, repaginare — vine din handlerul 'input')
            saveEditorSelection();
            scheduleEditorRepaginateIfNeeded(120);
        });
        function doPaste() {
            navigator.clipboard.readText().then(t => {
                ...
                focusPageNoJump();
                restoreEditorSelection();
                // runCmd → recordUndo + focus + selecție restaurată (deci Ctrl+Z funcționează)
        ...
        // ── Enter: paragraful nou moștenește formatarea celui curent (ca în Word) ──
        // La import, mărimea/culoarea textului stau într-un <span> din interiorul <p>
        // (docx pune font-size pe run, nu pe paragraf). Când caretul ajunge la capătul
        // paragrafului, Chrome îl lasă ADESEA în afara acelui <span>, așa că paragraful
        // creat cu Enter pornea cu fontul implicit al colii (Times 12pt) chiar dacă în
        // combo scria tot „Times New Roman 16". Construim noi blocul nou: același
        // tag/clasă/stil de paragraf + aceleași învelișuri inline (span/b/i/u…).
        const INLINE_FMT_TAGS = /^(SPAN|B|STRONG|I|EM|U|S|STRIKE|FONT|SUB|SUP|SMALL|MARK)$/;
        const ENTER_SPLIT_TAGS = /^(P|H[1-6]|DIV|BLOCKQUOTE)$/;
        // învelișurile inline „reale" — sar peste <a> (linkul nu se continuă în paragraful
        // nou) și peste marcajele temporare (dex-bad = cuvânt greșit, marker de paginare)
        function isCarriedInline(el) {
            return el.nodeType === 1 && INLINE_FMT_TAGS.test(el.tagName)
                && !el.classList.contains('dex-bad')
                && !el.hasAttribute('data-page-caret-marker');
        }
        function inlineChainAt(node, block) {
            const chain = [];
            let n = (node && node.nodeType === 3) ? node.parentNode : node;
            while (n && n !== block && n !== page) {
                if (isCarriedInline(n)) chain.unshift(n);
                n = n.parentNode;
            }
            return n === block ? chain : [];
        }
        // învelișurile de copiat în paragraful nou: cele de la caret, iar dacă acolo nu e
        // niciunul (caret „gol", lângă span, nu în el) → stilul dominant al paragrafului
        function inlineChainForNewBlock(block, caretNode) {
            const chain = inlineChainAt(caretNode, block);
            if (chain.length) return chain;
            const dom = dominantStyleElement(block);
            return (dom && dom !== block && block.contains(dom)) ? inlineChainAt(dom, block) : [];
        }
        function cloneInlineChainEmpty(chain) {
            let root = null, deepest = null;
            chain.forEach(el => {
                const c = el.cloneNode(false);   // fără conținut, doar tag + class + style
                c.removeAttribute('id');
                if (deepest) deepest.appendChild(c); else root = c;
                deepest = c;
            });
            return { root, deepest };
        }
        // paragraf gol, dar cu formatarea păstrată: <p …><span style="font-size:16pt"><br></span></p>
        function fillEmptyBlock(block, chain) {
            const { root, deepest } = cloneInlineChainEmpty(chain);
            const br = document.createElement('br');
            if (root) { deepest.appendChild(br); block.appendChild(root); }
            else block.appendChild(br);
        }
        function blockHasContent(node) {
            return node.textContent !== '' || !!node.querySelector('img,br,hr,table');
        }
        // caretul în interiorul învelișurilor (nu lângă ele), ca textul tastat să le moștenească
        function caretToBlockStart(block) {
            const t = document.createTreeWalker(block, NodeFilter.SHOW_TEXT).nextNode();
            const r = document.createRange();
            if (t) r.setStart(t, 0);
            else {
                // nu coborî ÎN <br>/<img> (elemente void): caretul acolo nu se poate serializa,
                // iar marcajul de repaginare s-ar pierde → cursorul sărea la începutul documentului
                let n = block;
                while (n.firstElementChild && !/^(BR|IMG|HR|INPUT|WBR)$/.test(n.firstElementChild.tagName)) n = n.firstElementChild;
                r.setStart(n, 0);
            }
            r.collapse(true);
            const s = window.getSelection(); s.removeAllRanges(); s.addRange(r);
        ...
        page.addEventListener('keydown', (e) => {
            ...
            // liste, celule de tabel, <pre> → comportamentul nativ al browserului e corect
            if (!block || !ENTER_SPLIT_TAGS.test(block.tagName) || block.closest('li,td,th')) return;
            // selecție peste mai multe paragrafe → lăsăm browserul să le unească
            if (!range.collapsed && blockOf(range.endContainer) !== block) return;

            e.preventDefault();
            recordUndo();
            if (!range.collapsed) range.deleteContents();
            const chain = inlineChainForNewBlock(block, range.startContainer);
            // tot ce e după caret trece în paragraful nou (extractContents păstrează
            // span-urile din care taie, deci formatarea cozii rămâne intactă)
            const tail = document.createRange();
            tail.setStart(range.startContainer, range.startOffset);
            tail.setEnd(block, block.childNodes.length);
            const frag = tail.extractContents();

            const nb = document.createElement(block.tagName);
            for (const at of block.attributes) if (at.name !== 'id') nb.setAttribute(at.name, at.value);
            if (blockHasContent(frag)) nb.appendChild(frag);
            else fillEmptyBlock(nb, chain);
            if (!blockHasContent(block)) fillEmptyBlock(block, chain);   // Enter la începutul paragrafului
            block.parentNode.insertBefore(nb, block.nextSibling);

            caretToBlockStart(nb);
        ...
        page.addEventListener('keydown', (e) => {
            ...
            if (d) {
                ...
                // execCommand nu declanșează 'beforeinput' → luăm noi snapshot-ul de undo
                noteTypingSnapshot();
                suppressSnap = true;
                try { document.execCommand('insertText', false, d); } finally { suppressSnap = false; }
            }
        });
        ...
        function setBookmarkHere() {
            ...
            bookmarkCycleKey = key;
            // A changed marker list starts a fresh F12 pass in document order.
        ...
        function clearBookmark(bookmarkId) {
            ...
            if (bookmarkId) {
                ...
                const removedIndex = before.findIndex(b => b.id === bookmarkId);
                const after = before.filter(b => b.id !== bookmarkId);
                saveActiveBookmarks(after);
                // The successor now occupies the deleted marker's index. F12 advances
                // once, so leave the cycle immediately before that slot (also on wrap).
                if (removedIndex >= 0) bookmarkCycleIndex = removedIndex - 1;
        ...
        function renderTabs() {
            ...
            updatePinBtn();   // butonul 📌 arată starea fișierului din tab-ul activ
        }
        ...
        // ── PIN: fișiere fixate ──
        // Butonul 📌 din bara de sus fixează fișierul deschis. Fixările stau în
        // localStorage, deci la fiecare pornire apar bifate deasupra „Fișiere recente"
        // (și în sidebar) — nu mai trebuie căutate prin foldere.
        // Se pot fixa doar fișierele cu cale pe disc; cele venite prin drag&drop nu au
        // cale, deci n-ar putea fi redeschise după repornirea aplicației.
        const PINNED_KEY = 'wordEditorPinned';   // [{path,name,ext,ts}]
        function getPinnedStore() { try { return JSON.parse(localStorage.getItem(PINNED_KEY)) || []; } catch (e) { return []; } }
        function setPinnedStore(list) { try { localStorage.setItem(PINNED_KEY, JSON.stringify(list)); } catch (e) { } }
        function pinKey(path) { return (path || '').replace(/\\/g, '/').toLowerCase(); }
        function isPinned(path) { const k = pinKey(path); return !!k && getPinnedStore().some(e => pinKey(e.path) === k); }
        // fișierul din tab-ul activ (sau cel deschis de pe disc)
        function currentPinEntry() {
            const t = curTab();
            const path = (t && t.path) || currentFile;
            if (!path) return null;
            const name = String((t && t.file) || path).split(/[\\/]/).pop();
            const ext = ((t && t.ext) || path.split('.').pop() || '').toLowerCase();
            return { path, name, ext };
        }
        function togglePinCurrent() {
            const entry = currentPinEntry();
            if (!entry) { toast('Deschide întâi un fișier de pe disc — PIN-ul reține calea lui'); return; }
            if (isPinned(entry.path)) { unpinFile(entry.path); toast('PIN scos: ' + entry.name); return; }
            const list = getPinnedStore().filter(e => pinKey(e.path) !== pinKey(entry.path));
            list.unshift({ path: entry.path, name: entry.name, ext: entry.ext, ts: Date.now() });
            setPinnedStore(list);
            refreshPinUi();
            toast('Fixat cu PIN: ' + entry.name);
        }
        function unpinFile(path) {
            setPinnedStore(getPinnedStore().filter(e => pinKey(e.path) !== pinKey(path)));
            refreshPinUi();
        }
        function refreshPinUi() { renderPinned(); renderSidebarPinned(); updatePinBtn(); }
        function updatePinBtn() {
            const btn = document.getElementById('pinBtn');
            if (!btn) return;
            const entry = currentPinEntry();
            btn.classList.toggle('pinned', !!entry && isPinned(entry.path));
        }
        function fileIcon(ext) {
            ext = (ext || '').toLowerCase();
            return ext === 'pdf' ? '📕' : ext === 'odt' ? '📘' : (ext === 'doc' || ext === 'rtf') ? '📃' : '📄';
        }
        // lista fixată din popup-ul de pornire: bifa = PIN pus; debifare = scoate PIN-ul
        function renderPinned() {
            const sec = document.getElementById('pinnedSection');
            const list = document.getElementById('pinnedList');
            if (!sec || !list) return;
            const items = getPinnedStore();
            sec.style.display = items.length ? 'block' : 'none';
            list.innerHTML = '';
            items.forEach(e => {
                const ext = (e.ext || e.name.split('.').pop() || '').toLowerCase();
                const folder = (e.path || '').replace(/\\/g, '/').split('/').filter(Boolean).slice(-2, -1)[0] || '';
                const row = document.createElement('div');
                row.className = 'pin-item'; row.title = e.path;
                const cb = document.createElement('input');
                cb.type = 'checkbox'; cb.checked = true; cb.title = 'Scoate PIN-ul';
                cb.onclick = (ev) => { ev.stopPropagation(); unpinFile(e.path); };
                const nm = document.createElement('span');
                nm.className = 'pn'; nm.textContent = fileIcon(ext) + ' ' + e.name;
                nm.onclick = () => openFile(e.path, ext);
                const fd = document.createElement('span');
                fd.className = 'pf'; fd.textContent = folder;
                row.appendChild(cb); row.appendChild(nm); row.appendChild(fd);
                list.appendChild(row);
            });
        }
        function renderSidebarPinned() {
            const box = document.getElementById('sbPinned');
            const hdr = document.getElementById('sbPinnedHdr');
            if (!box || !hdr) return;
            const items = getPinnedStore();
            box.innerHTML = '';
            hdr.style.display = items.length ? 'block' : 'none';
            items.forEach(e => {
                const ext = (e.ext || e.name.split('.').pop() || '').toLowerCase();
                const el = document.createElement('div');
                el.className = 'file-item'; el.title = e.path;
                el.innerHTML = '<span class="ico">📌</span><span class="fn">' + escapeHtml(e.name) + '</span>';
                el.onclick = () => openFile(e.path, ext);
                const x = document.createElement('span');
                x.className = 'unpin'; x.textContent = '✕'; x.title = 'Scoate PIN-ul';
                x.onclick = (ev) => { ev.stopPropagation(); unpinFile(e.path); };
                el.appendChild(x);
                box.appendChild(el);
            });
        }

        // ── Istoric fișiere (ultimele 25, ordonate după data deschiderii/închiderii) ──
        ...
        const MAX_RECENT = 25;                   // listele au scroll → cel puțin 20 la îndemână
        ...
        function renderRecent() {
            ...
            const hdr = document.getElementById('recentHdr');
            const items = allRecent();
            renderPinned();
            // panoul rămâne vizibil și când istoricul e gol, dacă există fișiere fixate
            panel.style.display = (items.length || getPinnedStore().length) ? 'block' : 'none';
            hdr.style.display = items.length ? 'block' : 'none';
            list.innerHTML = '';
            items.forEach(e => {
                const ext = (e.ext || e.name.split('.').pop() || '').toLowerCase();
        ...
        // ── Confirmare + backup la inchiderea ferestrei ──
        // Chrome nu permite un dialog propriu la X-ul ferestrei, dar cerand confirmarea
        // (preventDefault + returnValue) afisam intrebarea nativa "Leave site?" — asa nu
        // mai poti inchide din greseala un document nesalvat. Butonul „Iesire" ramane cu
        // dialogul nostru complet (Salveaza / Renunta / Anuleaza).
        window.addEventListener('beforeunload', e => {
            ...
            autoBackupBeacon();
            e.preventDefault();
            e.returnValue = '';     // Chrome cere returnValue setat ca sa afiseze dialogul
            return '';
        });
        ...
        renderRecent();
        renderSidebarRecent();
        refreshPinUi();   // fișierele fixate cu PIN, vizibile din prima
        showOverlay();    // popup de deschidere la pornire (ca în index V.4.php)
        setInterval(() => autoBackupNow(false), AUTO_BACKUP_INTERVAL_MS);
        // PWA instalabil → Chrome îi dă ID propriu de aplicație, iar iconița fixată în taskbar
        // devine chiar fereastra aplicației (nu mai apare o a doua iconiță)
